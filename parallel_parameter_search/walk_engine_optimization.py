...
class AbstractWalkEngine(AbstractWalkOptimization):
    ...
    def _suggest_walk_params(self, trial, trunk_height, foot_distance, foot_rise, trunk_x, z_movement):
        ...
        add('engine.foot_pitch_angle', 0.3, 1.0)
        add('engine.foot_pitch_acc', 5.0, 10.0)
        ...
        param_dict["node.phase_reset.effort.active"] = False
        param_dict["node.phase_reset.foot_pressure.active"] = False
        param_dict["node.phase_reset.imu.active"] = False
        ...
        param_dict["node.ik.reset"] = True
        param_dict["node.imu_y_acc_tau"] = 0.1
